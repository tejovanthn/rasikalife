...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (!institution) {
    // Not a teacher — nothing has been provisioned, because provisioning happens on the first
    // teaching write. Offer to start rather than 403: this is the "I am a guru" front door.
    return data({ institution: null, programs: [], includeArchived });
  }
  ...
  return data({ institution, programs, includeArchived });
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (intent === 'create-program') {
      ...
      return redirect(`/students/${program.id}`);
    }
...
export default function Roster() {
  const { institution, programs, includeArchived } = useLoaderData<typeof loader>();
  const actionData = useActionData<typeof action>();
  const navigation = useNavigation();

  if (!institution) {
    return (
      <Chrome title="Students" headerRight={<SignOutButton />}>
        <EmptyState
          title="Do you teach?"
          action={
            <Form method="post">
              <input type="hidden" name="intent" value="start-teaching" />
              <Button type="submit">Set up my classes</Button>
            </Form>
          }
        >
          Keep a shared record of who has paid for how many classes, and which ones have happened.
        </EmptyState>
      </Chrome>
    );
  }
  ...
  return (
    <Chrome title="Students" isTeacher headerRight={<SignOutButton />}>
      <div className="space-y-5">
        ...
        {programs.length === 0 ? (
          <EmptyState title="No classes set up yet">
            Add a weekly class or a workshop below, then add the students on it.
          </EmptyState>
        ) : (
