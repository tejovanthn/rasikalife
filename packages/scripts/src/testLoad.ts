...
const checkAndAdd = async ({ checker, adder, name }) => {
  const existing = await checker(name);
  if (existing.data.length > 0) return;
  return await adder({ name });
};

data.forEach(async song => {
  if (!song.title) return;

  const existingSong = await getSongByName(song.title);
  if (existingSong.data.length > 0) return;
  ...
  await addSong({
    ...
    id: undefined,
    name: song.title,
    slug: song.title.toLowerCase().replace(/ /g, '-'),
    raga: song.raga.join(','),
    source: `https://karnatik.com/${song.id}`,
  });

  if (song.raga)
    Promise.all(
      song.raga.map(async raga => {
        await checkAndAdd({
          checker: getRagaByName,
          adder: addRaga,
          name: raga,
        });
      })
  ...
  if (song.tala)
    await checkAndAdd({
      checker: getTalaByName,
      adder: addTala,
      name: song.tala,
    });
  if (song.composer)
    await checkAndAdd({
      checker: getComposerByName,
      adder: addComposer,
      name: song.composer,
    });
  if (song.tala)
    await checkAndAdd({
      checker: getLanguageByName,
      adder: addLanguage,
      name: song.language,
    });
});

getTop10Songs().then(console.log);
getTop10Ragas().then(console.log);
