...
export const meta: MetaFunction = ({ data: loaderData }) => {
  ...
  // Derive the landscape OG crop URL (generated by image-processor as *-og.jpg).
  // Falls back to the generic site image for events without a poster or uploaded before this feature.
  const posterUrl = event.posterUrl || ld?.festivalPosterUrl;
  const ogImage = posterUrl
    ? posterUrl.replace(/\.[^./?#]+($|\?)/, '-og.jpg$1')
