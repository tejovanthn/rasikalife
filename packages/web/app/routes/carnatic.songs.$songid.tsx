...
const Section = ({ heading, detail }: { heading: string; detail?: string }) => {
  ...
  return (
    <section className="mt-5">
      <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight first:mt-0">{heading}</h2>
      <p>{detail}</p>
    </section>
  )
}
...
export default function Index() {
  ...
  return (
    <main className="container">
      <h1 className="scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl">{data.title}</h1>
      <section className="mt-5">
        <ul>
