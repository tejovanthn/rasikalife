...
describe('QueryBuilder', () => {
  ...
  it('should query by partition key', async () => {
    const result = await createQuery().withPartitionKey('PK', 'USER#1').execute();
  ...
  it('should limit the number of results', async () => {
    const result = await createQuery().withPartitionKey('PK', 'USER#1').withLimit(2).execute();
  ...
  it('should implement pagination', async () => {
    // First page
    const resultPage1 = await createQuery().withPartitionKey('PK', 'USER#1').withLimit(2).execute();
  ...
  it('should throw an error when no key condition is specified', async () => {
    await expect(createQuery().execute()).rejects.toThrow('No key condition specified for query');
  });
