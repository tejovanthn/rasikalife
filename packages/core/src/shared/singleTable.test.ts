...
describe('Single Table Utilities', () => {
  ...
  it('should format date sort keys correctly', () => {
    expect(formatDateSortKey(SecondaryPrefix.DATE, '2023-01-01'))
      .toBe('DATE#2023-01-01');
    
    expect(formatDateSortKey(SecondaryPrefix.DATE, '2023-01-01', 'ITEM#123'))
      .toBe('DATE#2023-01-01#ITEM#123');
  });

  it('should format version keys correctly', () => {
    expect(formatVersionKey('v1')).toBe('VERSION#v1');
    expect(formatVersionKey('v1', '2023-01-01T00:00:00.000Z'))
      .toBe('VERSION#v1#2023-01-01T00:00:00.000Z');
  });
  ...
  it('should create composite IDs', () => {
    expect(createCompositeId(['USER', '123', 'POST', '456']))
      .toBe('USER_123_POST_456');
  });

  it('should format index keys', () => {
    expect(formatIndexKey('EMAIL', 'test@example.com'))
