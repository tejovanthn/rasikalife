...
export const contentNavigationSchema = z.object({
  ...
  childPaths: createArraySchema(createStringSchema({ maxLength: 500 }), { maxItems: 50 }).optional(),
}).strict();
...
export const createContentSchema = z.object({
  path: z.string().min(1).max(500).regex(/^\/[a-z0-9\-\/]*$/, 'Path must start with / and contain only lowercase letters, numbers, hyphens, and forward slashes'),
  content: createStringSchema({ minLength: 1 }),
