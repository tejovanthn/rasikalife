...
function mockImplementPutCommand(command: any) {
  ...
  const existingIndex = table.findIndex(item => item.PK === Item.PK && item.SK === Item.SK);
...
function mockImplementGetCommand(command: any) {
  ...
  const item = table.find(item => item.PK === Key.PK && item.SK === Key.SK);
...
function mockImplementUpdateCommand(command: any) {
  ...
  const existingIndex = table.findIndex(item => item.PK === Key.PK && item.SK === Key.SK);
  ...
  if (UpdateExpression.includes('SET')) {
    const setExpressions = UpdateExpression.split('SET ')[1]
      .split(',')
      .map(expr => expr.trim());
...
function mockImplementQueryCommand(command: any) {
  ...
  if (KeyConditionExpression) {
    ...
    if (resolvedExpression.includes('SK = :SK') && ExpressionAttributeValues[':SK']) {
      ...
      resolvedExpression.includes('begins_with(SK, :') &&
      resolvedExpression.match(/begins_with\(SK, :([^\)]+)\)/)
    ) {
      const match = resolvedExpression.match(/begins_with\(SK, :([^\)]+)\)/);
      if (match && ExpressionAttributeValues[`:${match[1]}`]) {
        const prefix = ExpressionAttributeValues[`:${match[1]}`];
        filteredItems = filteredItems.filter(
          item => typeof item.SK === 'string' && item.SK.startsWith(prefix)
        );
      }
      ...
      filteredItems = filteredItems.filter(item => item.SK >= startValue && item.SK <= endValue);
    }
    ...
    if (IndexName && resolvedExpression.includes(`${IndexName}SK`)) {
      ...
      // begins_with on SK
        ...
        const match = resolvedExpression.match(
          new RegExp(`begins_with\\(${IndexName}SK, :(\\w+)\\)`)
        );
        if (match && ExpressionAttributeValues[`:${match[1]}`]) {
          const prefix = ExpressionAttributeValues[`:${match[1]}`];
          filteredItems = filteredItems.filter(
            item =>
              typeof item[`${IndexName}SK`] === 'string' &&
              item[`${IndexName}SK`].startsWith(prefix)
          );
        }
      }
      // between on SK
        ...
        const match = resolvedExpression.match(
          new RegExp(`${IndexName}SK BETWEEN :(\\w+) AND :(\\w+)`)
        );
        if (
          match &&
          ...
          filteredItems = filteredItems.filter(
            item => item[`${IndexName}SK`] >= startValue && item[`${IndexName}SK`] <= endValue
          );
        }
      }
    }
  ...
  filteredItems.sort((a, b) => {
    ...
    return ScanIndexForward ? (a[sortKey] > b[sortKey] ? 1 : -1) : a[sortKey] < b[sortKey] ? 1 : -1;
  });
...
function mockImplementScanCommand(command: any) {
  const { TableName, FilterExpression, ExpressionAttributeValues, Limit, ExclusiveStartKey } =
    command;
...
function mockImplementTransactWriteCommand(command: any) {
  ...
  for (const item of TransactItems) {
    if (item.Put) {
      ...
      // Check if item with the same key already exists
      const existingIndex = table.findIndex(i => i.PK === Item.PK && i.SK === Item.SK);
      ...
      const existingIndex = table.findIndex(i => i.PK === Key.PK && i.SK === Key.SK);
