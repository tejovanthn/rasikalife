...
- `packages/core`: **871 tests pass, 3 fail** (`updateArtist`/`updateRaga`/`updateTala` "should throw error when update fails" — all three assert a capitalised message the code emits lowercase; confirmed failing identically at HEAD); **2 files with typecheck errors** (`edit/service.ts`, `event/index.ts:46` — a `festivalId` null). Was 795 pass before this work; +76 for the affiliation junction, the widened artist schema, the completion labels, the extraction pipeline, and the two review passes.
