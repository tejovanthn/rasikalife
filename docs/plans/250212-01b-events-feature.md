# Events Feature V2 - MVP Specification

**Version:** 2.0 (DHH Feedback Applied)  
...
The core value proposition remains unchanged: automate event data entry while ensuring accuracy through human verification. What changed is the implementation—four entities became one, thirteen GSIs became three, and the complex async pipeline became a simple synchronous flow.
...
export class EventService {
  ...
  static async createFromPoster(input: z.infer<typeof CreateEventFromPosterInputSchema>, userId: string): Promise<Event> {
    ...
    const event = await EventEntity.create({
      ...
      gsi2sk: extracted.startDateTime || new Date().toISOString(),
...
- DHH Feedback: `/docs/plans/250212-01a-events-feature-dhh-feedback.md`
