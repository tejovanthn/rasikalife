...
**Key changes from v1:**
- Folded into `packages/core/src/domain/search` (no new package)
- Removed 5-minute cache TTL; use in-memory caching forever with version check
- Simplified response types (`id`, `type`, `name`, `highlights` only)
- Added health endpoint with freshness validation (fail if >24 hours old)
- Single Zod schema as source of truth for filter fields
...
├── service.ts            # Search service with caching
├── index.ts              # Lambda handler for search
└── cron/
    └── refresh-index.ts  # Cron job handler for index rebuild
```
...
import type { SearchDocument, SearchableField } from './types';
import { SearchableFieldSchema } from './schema';

export function createFuseOptions(filterFields?: SearchableField[]): Fuse.IFuseOptions<SearchDocument> {
...
function transformEntity(entity: { id: string }, entityType: EntityType): SearchDocument {
  return {
    id: entity.id,
    entityType,
    artistName: '',
...
export async function buildSearchIndex(): Promise<SearchIndex> {
  ...
  logger.info(`Fetched entities: ${artists.length} artists, ${ragas.length} ragas, ${talas.length} talas, ${compositions.length} compositions`);
  ...
  logger.info(`Search index built: ${documents.length} documents`);
...
export async function storeSearchIndex(index: SearchIndex): Promise<void> {
  ...
  logger.info(`Search index stored: ${indexKey} (latest pointer updated)`);
}
...
import { S3Client } from '@aws-sdk/client-s3';
import type { SearchDocument, SearchIndex, SearchResultItem, SearchResponse, HealthStatus } from './types';
...
export async function search(query: string, options: SearchOptions = {}): Promise<SearchResponse> {
  ...
  try {
    // Load index (with infinite cache)
    const index = await loadIndex();
    const documents = index.documents;

    // Create Fuse instance with pre-built index
    const fuseOptions = createFuseOptions(filters as never[]);
...
async function loadIndex(): Promise<SearchIndex> {
  ...
  try {
    ...
    logger.info('Search index loaded from S3', {
...
export async function getHealth(): Promise<HealthStatus> {
  try {
    ...
    if (indexAgeHours > 24) {
      logger.warn('Search index is stale', { indexAgeHours });
      return {
        status: 'stale',
...
import { search, getHealth } from './service';
import { logger } from '@/logging';

export const handler: APIGatewayProxyHandler = async (event) => {
  const requestId = event.requestContext.requestId;

  logger.info('Search request received', { requestId, path: event.path });
  ...
  try {
    ...
    const result = await search(query, { filters, limit, offset });
    ...
    logger.error('Search handler error', { requestId, error });
...
export async function handler() {
  ...
  try {
    ...
    logger.error('Scheduled index refresh failed', { error });
...
3. **Define types and schemas**
   - Create `types.ts` with SearchDocument, SearchIndex, SearchResultItem, HealthStatus
...
7. **Implement Fuse.js integration**
   - Create `service.ts` with createFuseOptions
   - Configure fuzzy matching parameters
   - Handle diacritics for Indian languages

8. **Implement search service**
   - Create search() function with infinite in-memory caching
   - Implement S3 index loading with ETag/version check
...
13. **Unit tests for transformer**
    - Test document transformation for each entity type

14. **Unit tests for service**
    - Test Fuse.js options
    - Test search with various queries
    - Test filter behavior
    - Test pagination
    - Test health check freshness validation

15. **Integration tests**
    - Test full search flow with mock data
    - Test index build process
    - Test error scenarios (S3 failure, stale index)
...
import { search, getHealth, createFuseOptions } from './service';
...
| Filters | String enum + type duplication | Single Zod schema (`SearchableFieldSchema`) as source of truth |
| Logging | Missing | Structured logging (no metrics in response) |
